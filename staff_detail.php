...
// Ambil target (guru/kepsek/yayasan/admin)
$stmt = $mysqli->prepare("SELECT id, full_name, role, employee_no, photo_path, academic_year_id FROM users WHERE id=? LIMIT 1");
$stmt->bind_param("i", $target_id);
$stmt->execute();
$target = $stmt->get_result()->fetch_assoc();
if (!$target) { flash_set("warning","User tidak ditemukan."); redirect("/dashboard.php"); }
...
<div class="card card-soft p-3">
  <div class="d-flex align-items-center justify-content-between">
    <div>
      <h5 class="fw-semibold mb-0"><i class="bi bi-search me-1"></i>Detail Absensi Staff</h5>
      <div class="text-secondary small"><?= e($target["full_name"]) ?> • <?= e(role_label($target["role"])) ?><?= !empty($target["employee_no"]) ? " • " . e($target["employee_no"]) : "" ?></div>
      <div class="text-secondary small">Periode: <?= e(indonesian_date($from)) ?> s/d <?= e(indonesian_date($to)) ?></div>
    </div>
    <div class="d-flex gap-2">
      <button class="btn btn-sm btn-outline-primary" onclick="window.print()"><i class="bi bi-printer me-1"></i>Print</button>
      <div class="d-flex gap-2">
  <a class="btn btn-sm btn-outline-primary" target="_blank" href="<?= $BASE_URL ?>/staff_rekap_print.php?staff_id=<?= (int)$target_id ?>&from=<?= e($from) ?>&to=<?= e($to) ?>">
    <i class="bi bi-printer me-1"></i>Print Rekap
  </a>
  <a class="btn btn-sm btn-outline-success" href="staff_detail_export.php?id=<?= (int)$target_id ?>&from=<?= e($from) ?>&to=<?= e($to) ?>"><i class="bi bi-file-earmark-excel me-1"></i>Excel</a>
...
function openDetail(title, photoUrl, lat, lng){
  ...
  if(lat && lng){
    ...
    L.marker([parseFloat(lat), parseFloat(lng)]).addTo(mapObj);
  } else {
