...
if ($staff_id <= 0) die("staff_id tidak valid");

// akses: admin/kepsek/yayasan boleh print siapa saja, guru boleh print dirinya sendiri
if (!in_array($u["role"], ["ADMIN","KEPSEK","YAYASAN","GURU"], true)) die("Akses ditolak");
if ($u["role"] === "GURU" && (int)$u["id"] !== $staff_id) die("Akses ditolak");

// ambil sekolah
$school = $mysqli->query("SELECT * FROM school_profile WHERE id=1")->fetch_assoc();

// ambil staff (guru/kepsek/yayasan)
$stmt = $mysqli->prepare("SELECT u.id,u.full_name,u.employee_no,u.role,u.photo_path,u.academic_year_id,
  ay.name AS year_name
  FROM users u
  LEFT JOIN academic_years ay ON ay.id=u.academic_year_id
  WHERE u.id=? AND u.role IN ('GURU','KEPSEK','YAYASAN') LIMIT 1");
$stmt->bind_param("i", $staff_id);
$stmt->execute();
$staff = $stmt->get_result()->fetch_assoc();
if (!$staff) die("Data staff tidak ditemukan");
...
$ay_id = (int)($staff["academic_year_id"] ?? get_active_academic_year_id($mysqli));
...
<head>
  ...
  <title>Rekap Absensi Staff</title>
  <style>
...
<div class="wrap">
  ...
  <h1>SURAT KETERANGAN REKAP ABSENSI STAFF</h1>
  <div class="sub">Periode: <?= e(indonesian_date($from)) ?> s/d <?= e(indonesian_date($to)) ?></div>

  <table class="meta">
    <tr><td width="120">Nama</td><td>: <?= e($staff["full_name"]) ?></td></tr>
    <tr><td>Jabatan</td><td>: <?= e(role_label($staff["role"])) ?></td></tr>
    <tr><td>No. Pegawai</td><td>: <?= e($staff["employee_no"] ?? "-") ?></td></tr>
    <tr><td>Tahun Pelajaran</td><td>: <?= e($staff["year_name"] ?? "-") ?></td></tr>
  </table>
    ...
    <tbody>
      ...
      <tr>
        ...
        <td>
          ...
          <?php elseif ($l): ?>
            <div class="small">Status Ijin: <?= e($l["status"] ?? "-") ?></div>
  ...
  <div class="ttd">
    <div class="box">
      ...
      <div style="margin-top:8px;"><?= e(role_label($u["role"])) ?></div>
